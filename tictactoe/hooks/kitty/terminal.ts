...
/** Row-number diacritics, from kitty's rowcolumn-diacritics.txt, in order. */
...
export const MAX_ROWS = ROW_DIACRITICS.length

/**
 * One text row of placeholders: the first cell names its row, and each later
 * cell takes the next column from the cell before it.
 */
export function placeholderRow(row: number, columns: number): string {
  const diacritic = String.fromCodePoint(ROW_DIACRITICS[row] ?? 0x0305)

  return `${PLACEHOLDER}${diacritic}${PLACEHOLDER.repeat(Math.max(0, columns - 1))}`
}
